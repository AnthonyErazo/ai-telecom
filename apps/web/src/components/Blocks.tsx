...
export function Blocks({ blocks }: { blocks: Block[] }) {
  return <div className="blocks">{blocks.map((block, index) => {
    ...
    if (block.tipo === "texto") return <article className={block.enfasis ? "block emphasis" : "block"} key={key}>{block.titulo && <h3>{block.titulo}</h3>}<p>{conNegrita(block.texto)}</p></article>;
